	...
	while (head != NULL)
	{
		...
		else
		{
			...
			if (q->runcount == 1)// 该进程第一次运行
			{
				...
				temp.endtime = 0;
				temp.next = NULL;
				if (vec_out.size() != 0 && vec_out[vec_out.size() - 1].endtime == 0)
				{
			...
			++q->runcount;
			--q->resttime;
			if (q->resttime == 0)// 该进程运行结束
			{
				// 记录结束时间
				q->endtime = time;
				// 计算周转时间
				q->zhouzhuangtime = time - q->arrivetime;
				// 计算带权周转时间
				q->weightzhouzhuangtime = q->zhouzhuangtime / (double)q->runtime;
				weighttotaltime += q->weightzhouzhuangtime;
				// 输出信息
				temp = *q;
				temp.starttime = 0;
				temp.next = NULL;
