...
static void line_fill(char* line, int len, char** words, int* word_lens, int max_size,int even_spaces, int remain_spaces, int start, int end)
{
    int i, j;
    char* p = line;
    for (i = start; i < end; i++)
...
static char** fullJustify(char** words, int wordsSize, int maxWidth, int* returnSize)
{
    ...
    char* buf = (char*)malloc(cap * (maxWidth + 1));
    for (i = 0; i < cap; i++)
    {
